...
async function initHomePage() {
    ...
    const areas = areaData.results.map(a => a.name)
...
recipesGrid.addEventListener("click", async (e) => {
    ...
    servings = analyzedData.servings
    ...
    console.log('meals name', mealName);
  ...
  <div class="flex items-center gap-4">
    ...
    <div>
      ...
      <p class="text-sm text-gray-500">
        ...
        <span class="${item.type === "product" ? "text-blue-600" : "text-emerald-600"}">
          ${item.type === "product" ? "Product" : "Recipe"}
        </span>
      </p>
...
function buildWeeklySummary() {
    ...
    foodLog.forEach(item => {
        ...
        summary[dateKey].calories += item.nutrition.calories;
...
categoryButtons.forEach(btn => {
    btn.addEventListener("click", async () => {
        activeCategory = btn.dataset.productcategory; 
